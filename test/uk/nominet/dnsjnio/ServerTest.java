...
//    final static int PORT = 53;
    ...
    public void testTimeoutsAsync() throws Exception {
        ...
        if (!(result.getException() instanceof SocketTimeoutException)) {
            fail("Exception " + result.getException() + " thrown instead of SocketTimeoutException!");
        }
    }
    ...
    private void doTestManySequentialAsynchronousRequests(NonblockingResolver resolver) throws TextParseException {
        int numRequests = 100;
...
//            System.out.println("Got back " + i + ", " + (result.getException() instanceof SocketTimeoutException ? "timed out" : "OK"));
    ...
    private void doTestManyAsynchronousRequests(NonblockingResolver resolver) throws Exception {
        doTestManyAsynchronousRequests(resolver, false);
    }
    private void doTestManyAsynchronousRequests(NonblockingResolver resolver, boolean useSameHeaderId) throws Exception {
        int numRequests = 500;
//        String name = "example.net";
...
//                System.out.println(i+ ", Result " + response.getId() + " threw Exception " + response.getException());
            }
            assertTrue(!response.isException() || response.getException() != null);
            if (response.isException()) {
                bad++;
            }
        }
        assertTrue("Too many exceptions! (" + bad + " of " + numRequests + ")", bad < (numRequests * 0.1));
    }
    ...
    public void testManyAsynchronousClients() throws Exception {
        ...
        for (int i = 0; i < numClients; i++) {
            ...
            resolver.setTimeout(TIMEOUT);
                        ...
                        Message query = getQuery("example" + ((Integer) (id)).intValue() + ".net");
                        resolver.sendAsync(query, id, queue);
                    } catch (Exception e) {}
                }
            };
...
//                System.out.println("Result " + response.getId() + " threw Exception " + response.getException());
            }
            assertTrue(!response.isException() || response.getException() != null);
            if (response.isException()) {
                bad++;
    ...
    private void doTestManySequentialSynchronousRequests(NonblockingResolver resolver) throws TextParseException {
        int numRequests = 50;
        resolver.setTimeout(TIMEOUT);
        resolver.setPort(PORT);
        int bad = 0;
        for (int i = 0; i < numRequests; i++) {
            ...
            try {
                ...
                if (!ret.getQuestion().getName().toString().equals(name)) {
                    fail("Name should be " + name + " but is " + ret.getQuestion().getName().toString());
                }
...
//                    System.out.println(i + " timed out");
    ...
    public void testQueryTimeout() throws Exception {
        ...
        // To test the timeout, set the Resolver timeout to be 100, and the query timeout to be 1.
        NonblockingResolver resolver = new NonblockingResolver("localhost");
        resolver.setPort(PORT);
        resolver.setTimeout(TIMEOUT);
        Message query = getQuery("timeout.example.net");
        ...
        assertTrue(time < 100); // Check that query timeout of 1 was used, not resolver timeout of 100
        if (!result.isException()) {
    ...
    public void testManyThreadedSynchronousClients() throws Exception {
        ...
        for (int i = 0; i < numThreads; i++) {
            SynchronousQueryThread thread = new SynchronousQueryThread("example" + i + ".net", TIMEOUT, new Integer(idCount++));
            threads.add(thread);
        }
...
//                System.out.println("Result " + thread.id + " threw Exception");
                bad++;
            }
            if (thread.x != null) {
        ...
        assertTrue("Too many exceptions! (" + bad + " of " + numThreads + ")", bad < (numThreads * 0.05));
    }
    ...
    public class SynchronousQueryThread extends Thread {
        ...
        public SynchronousQueryThread(String name, int timeout, Object id) throws Exception {
            this.name = name;
            this.id = id;
